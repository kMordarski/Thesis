...
R = \begin{bmatrix} 
    ...
    \end{bmatrix}
,\]
where $R$ is a matrix of the results, $r_{NM}$ is a coop/def proportion after $2000 + M$ generations in $N$th simulation. Now we may denote $R$ as:
\[
  P = (R_1, \dots, R_{20}),
\]
...
Applying the above procedure to our vector results in gaining 20 data points, and allows us to produce one plot for each Barabasi-Albert Model parametriation. 

Since we have three such parametrizations for each game, we gain 6 plots of coop/def tracks. We've decided to group them in regard of games so that they could be easly compared. 

\subsection{Discussion}

The results seem to agree with the general intuition concerning the game theory. As we analyzed PD and SD games in SECTION 4, we highlighted the Nash equilibriums and optimal, in this sense, strategies. 

For the PD game, we demonstrated that for the lowest value of $b$ parameter (equal to 1), players are indifferent to the choice of their strategy. We may observe that on the plots FIG, the value for $b=1$ is close to $0.5$. As the value of $b$ increases, we can see that the average share of cooperators is decreasing. This is also a reasonable result --- $b$ parameter influences the dominance of $Defect$ strategy amongst the population of players. Thus, the higher the $b$ parameter is, the more competitive the $defect$ strategy becomes. 
...
Throughout the course of this paper we have managed to accomplish --- to some extent, both of the goals set in the first chapter. 
